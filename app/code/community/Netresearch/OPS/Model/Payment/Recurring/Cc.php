        ...
        switch ($profile->getNewState()) {
            case Mage_Sales_Model_Recurring_Profile::STATE_ACTIVE:
                if (Mage::getSingleton('admin/session')->isLoggedIn()) {
                    $this->addAdminNotice('To actually activate the subscription an update in the PayEngine backend is needed.'
                    );
                } else {
                    Mage::throwException(
            ...
            case Mage_Sales_Model_Recurring_Profile::STATE_CANCELED:
                if (Mage::getSingleton('admin/session')->isLoggedIn()) {
                    $this->addAdminNotice('To actually cancel the subscription an update in the PayEngine backend is needed.'
                    );
                } else {
            ...
            case Mage_Sales_Model_Recurring_Profile::STATE_SUSPENDED:
                if (Mage::getSingleton('admin/session')->isLoggedIn()) {
                    $this->addAdminNotice('To actually suspend the subscription an update in the PayEngine backend is needed.'
                    );
                } else {
                    $this->sendSuspendMail($profile);
                }
